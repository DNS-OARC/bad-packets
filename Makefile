...
v4_frag_fuzz_offset.pcap: v4_frag_fuzz_offset.py
	PYTHONPATH="$(PWD)/scapy" ./$< "$(DESTDIR)/$@" $(NUM_PKTS) $(FRAG_PKT_SIZE) $(FRAG_SIZE)
...
v4_frag_skip_first.pcap: v4_frag_skip.py
	PYTHONPATH="$(PWD)/scapy" ./$< "$(DESTDIR)/$@" $(FRAG_PKT_SIZE) $(FRAG_SIZE) 0

v4_frag_skip_middle.pcap: v4_frag_skip.py
	PYTHONPATH="$(PWD)/scapy" ./$< "$(DESTDIR)/$@" $(FRAG_PKT_SIZE) $(FRAG_SIZE) $(FRAG_SKIP_MIDDLE)

v4_frag_skip_last.pcap: v4_frag_skip.py
	PYTHONPATH="$(PWD)/scapy" ./$< "$(DESTDIR)/$@" $(FRAG_PKT_SIZE) $(FRAG_SIZE) -1


v4_frag_offset_offbyone1.pcap: v4_frag_offset.py
	PYTHONPATH="$(PWD)/scapy" ./$< "$(DESTDIR)/$@" $(FRAG_PKT_SIZE) $(FRAG_SIZE) $(FRAG_OFFBYONE_PLUS) 1

v4_frag_offset_offbyone2.pcap: v4_frag_offset.py
	PYTHONPATH="$(PWD)/scapy" ./$< "$(DESTDIR)/$@" $(FRAG_PKT_SIZE) $(FRAG_SIZE) $(FRAG_OFFBYONE_MINUS) -1


v4_frag_empty.pcap: v4_frag_empty.py
	PYTHONPATH="$(PWD)/scapy" ./$< "$(DESTDIR)/$@" $(FRAG_PKT_SIZE) $(FRAG_SIZE) $(FRAG_EMPTY)


v4_frag_dup.pcap: v4_frag_dup.py
	PYTHONPATH="$(PWD)/scapy" ./$< "$(DESTDIR)/$@" $(FRAG_PKT_SIZE) $(FRAG_SIZE) $(FRAG_DUP)


v4_frag_order.pcap: v4_frag_order.py
	PYTHONPATH="$(PWD)/scapy" ./$< "$(DESTDIR)/$@" $(FRAG_PKT_SIZE) $(FRAG_SIZE)


v4_frag_nomf.pcap: v4_frag_nomf.py
	PYTHONPATH="$(PWD)/scapy" ./$< "$(DESTDIR)/$@" $(FRAG_PKT_SIZE) $(FRAG_SIZE) $(FRAG_NOMF)
